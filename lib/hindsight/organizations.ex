defmodule Hindsight.Organizations do
  @moduledoc """
  ...
  alias Hindsight.Organizations.Organization
  ...
  def free_plan_decision_limit_reached(organization) do
    if organization.status == "free" do
      Repo.aggregate(from(r in Hindsight.Decisions.Decision, where: r.organization_id == ^organization.id), :count, :id) >= 50
    else
      false
    end
  end
  ...
  def run_weekly_roundup do
    ...
    Enum.map(organizations, fn organization ->
      Enum.map(organization.users, fn user ->
        Hindsight.Email.weekly_roundup(user.email, Hindsight.Decisions.get_weeky_decisions_for_user(user))
