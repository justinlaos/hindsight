defmodule HindsightWeb.SignupController do
  ...
  def create_organization(conn, params) do
    ...
    case Hindsight.Signup.create_organization(email, password, promo) do
      {:ok, user} ->
        conn
        |> Pow.Plug.authenticate_user(params["user"])
        |> case do
          {:ok, conn} ->
            Hindsight.Data.event(conn.assigns.current_user, "Signed Up")
            conn
            |> put_flash(:info, "Welcome back!")
            |> redirect(to: Routes.decision_path(conn, :index))
          end
      {:error, error} ->
        Hindsight.Data.signup_attempt_event(email, error)
